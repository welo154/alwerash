...
export function MentorPhotoUpload({ mentorId, photo }: Props) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/mentors/${mentorId}/photo`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex h-24 w-24 shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
          {photo ? (
            <Image
              src={photo}
              alt="Mentor"
