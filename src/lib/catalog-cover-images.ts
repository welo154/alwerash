...
const DEFAULT_COURSE_IMAGE =
  "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=700&h=760&fit=crop&q=80";

const DEFAULT_TRACK_IMAGE =
  "https://images.unsplash.com/photo-1634942537034-2531766767d1?w=800&h=500&fit=crop&q=80";

function unsplash(photoId: string, w: number, h: number): string {
  return `https://images.unsplash.com/${photoId}?w=${w}&h=${h}&fit=crop&q=80`;
}

/** Legacy IDs that 404 on Unsplash — ignore stored URLs containing these. */
const BROKEN_UNSPLASH_IDS = [
  "photo-1561070791-2526d38794a5",
  "photo-1583485088134-034bcb65a0b7",
  "photo-1558591710-4b4c1c0e5e0a",
  "photo-1455390572244-0443dd647dd0",
  "photo-1579783902617-a3fb3927b6a5",
  "photo-1513364776144-ebeb3fe3d8fe",
  "photo-1541701494587-ab638cfee0b6",
...
function isBrokenCoverUrl(url: string | null | undefined): boolean {
  const v = url?.trim();
  if (!v) return true;
  if (!v.startsWith("http") && !v.startsWith("/")) return true;
  return BROKEN_UNSPLASH_IDS.some((id) => v.includes(id));
}
...
const TRACK_COVER_BY_SLUG: Record<string, string> = {
  "design-fundamentals": unsplash("photo-1634942537034-2531766767d1", 800, 500),
  ...
  lettering: unsplash("photo-1596464716127-f2a82984de30", 800, 500),
  ...
  calligraphy: unsplash("photo-1596464716127-f2a82984de30", 800, 500),
  ornamentation: unsplash("photo-1558618666-fcd25c85cd64", 800, 500),
  "apparel-design": unsplash("photo-1521572163474-6864f9cf17ab", 800, 500),
  "illustration-and-drawing": unsplash("photo-1547891654-e66ed7ebb968", 800, 500),
  ...
  "ui-ux": unsplash("photo-1517694712202-14dd9538aa97", 800, 500),
  "ui-ux-design": unsplash("photo-1517694712202-14dd9538aa97", 800, 500),
  ...
  ai: unsplash("photo-1555949963-aa79dcee981c", 800, 500),
  figma: unsplash("photo-1517694712202-14dd9538aa97", 800, 500),
  za5rafa: unsplash("photo-1634942537034-2531766767d1", 800, 500),
};
...
const COURSE_KEYWORD_COVERS: { match: RegExp; url: string }[] = [
  ...
  { match: /figma/i, url: unsplash("photo-1517694712202-14dd9538aa97", 700, 760) },
  ...
  { match: /calligraph|naskh|ruqaa|thuluth|diwani|kufi|script/i, url: unsplash("photo-1596464716127-f2a82984de30", 700, 760) },
  { match: /lettering|typograph|type design|glyphs|font/i, url: unsplash("photo-1596464716127-f2a82984de30", 700, 760) },
  ...
  { match: /illustrat|character|storyboard|graffiti/i, url: unsplash("photo-1547891654-e66ed7ebb968", 700, 760) },
  { match: /ornament|pattern|geometric|batik|floral/i, url: unsplash("photo-1558618666-fcd25c85cd64", 700, 760) },
...
export function resolveTrackCoverImage(
  ...
  if (stored && !isBrokenCoverUrl(stored)) return stored;
  const slug = trackSlug?.trim().toLowerCase();
...
export function resolveCourseCoverImage(input: {
  ...
  const stored = input.coverImage?.trim();
  if (stored && !isBrokenCoverUrl(stored)) return stored;
...
export function catalogCoverNeedsRefresh(
  stored: string | null | undefined,
  computed: string
): boolean {
  const current = stored?.trim();
  if (!current) return true;
  if (isBrokenCoverUrl(current)) return true;
  return current !== computed;
}
/** URLs used when backfilling the database (same logic as runtime fallbacks). */
export function catalogCoverUrlForTrack(slug: string, _title?: string): string {
